...
export class ListComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.routeSub = this.route.queryParams.subscribe((queryParams) => {
      ...
      if (queryParams['q']) {
        this.getAllInfra({ q: queryParams['q'] });
      }
      else if (Object.keys(this.params).length > 0) {
        this.getAllInfra(queryParams);
      }
      else{
        this.getAllInfra();
      }
  ...
  getAllInfra(params = null) {
    const sub = this._productServices.getAllFurniture('Infrastructure', params).subscribe({
      next: (res) => {
  ...
  getGraphData() {
    const sub = this._productServices.getGraphData('Infrastructure').subscribe({
      next: (res) => {
