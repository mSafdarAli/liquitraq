...
export class AuthService {
  ...
  public login(email: string, password: string) {
    return this.http
      ...
      }>(environment.api + 'auth/login', { email, password })
      .pipe(
        map((res) => {
          if (res && res.token) {
  ...
  public forgotPassword(userName: string) {
    return this.http
      .post(environment.api + 'auth/resetPassword', { userName })
      .pipe(
        map((res) => {
          return res;
        })
  ...
  public registerUser(data: Object) {
    return this.http.post(environment.api + 'auth/register', data).pipe(
      map((res) => {
        return res;
      })
    );
